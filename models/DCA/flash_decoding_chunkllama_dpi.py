...
class ChunkLlamaRotaryEmbedding(nn.Module):
    ...
    def __init__(self, dim=None, max_position_embeddings=4096, base=10000, device=None, scaling_factor=1.0, rope_type="default", config = None): # add by dpi
        super().__init__()
        # modified by dpi
        self.config = config
        self.max_seq_len = config.max_position_embeddings
        self.dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
...
def forward(
    ...
    chunk_len = chunk_size - local_window
    # print("attn forward, q_len: ",q_len)
    ...
    if not has_kv_cache:
        ...
        # print("cache_seqlens 2 ",cache_seqlens, kv_seq_len, chunk_len, chunk_num_curr)
...
def LlamaModel_forward(
    ...
    # print("len(past_key_values) ",len(past_key_values) if past_key_values is not None else 0)
    ...
    self._use_flash_attention_2 = True # add by dpi
...
def causal_forward(self,
                   ...
                   output_hidden_states: Optional[bool] = None,
                   return_dict: Optional[bool] = None,
                    cache_position: Optional[torch.LongTensor] = None, # add by dpi
    ...
    if hidden_states.shape[-2] < full_logits_length:
        ...
        logits = logits.float()
        # print("hidden_states ",hidden_states.shape, logits.shape)
        loss = None
        ...
        if labels is None:
            ...
            loss = None
            # print("hidden_states 2",hidden_states.shape, logits.shape)
        else:
    ...
    if getattr(self, "cache_all_logits", False) == True:
        logits_cache = getattr(self, "logits_cache", ())
        logits_cache += (logits.clone().float().to('cpu'),)
        setattr(self, "logits_cache", logits_cache)
...
def replace_with_chunkllama(pretraining_length=4096, local_window_size=None, max_prompt_length=None, max_pe=None):
    global chunk_size
    global local_window
